...
/**
 * The {@link RedisWrapper} is a wrapper around the {@link redis.clients.jedis.JedisPool}.
 * It leaves all thread management to the user and simply does as told. It provides methods
 * for pub/sub handling, as well as basic caching.
 */
public interface RedisWrapper {

  int DEFAULT_PORT = 6379;

  static RedisWrapper newWrapper(String address, int port) {
    return new RedisWrapperImpl(address, port);
  }
